...
const FormWebPage = ({ pageData = null }) => {
    ...
    const filteredSubCategories = CategorySubcategory?.filter(subCategory => subCategory.slug === selectedCategory);
    ...
    useEffect(() => {
        if (pageData) {
            if (pageData.allPro === 0) {
                ...
                if(!pageData.catSlug && !pageData.subCatSlug) setValue('allPro', pageData.allPro);
            }
            ...
            const images = pageData?.pageImages && pageData?.pageImages?.map((image) => ({
    ...
    const validateForm = (data) => {
        if (!value.trim()) return showError('Oops! Looks like you forgot to add a description. Don’t be shy!');
        if (!data.title) return showError('Title missing! Every epic needs a title, right?');
        if (data.allPro !== 0) {
            if (data.allPro === 2 && !data.catSlug) return showError('Category is a must! Don’t leave it hanging.');
            if (data.allPro === 3 && (!data.catSlug || !data.subCatSlug)) return showError('Pick a category and a subcategory. It’s not that hard!');
            if (!data.minPrice || !data.maxPrice) return showError('Price range please! Every thing have a limit');
            if (data.minPrice > data.maxPrice) return showError('When did min become greater than max? Genius!');
        }
        return true;
    };
    ...
    const submitFormHandler = async (data) => {
        ...
        try {
            ...
            const booleanValue = (data?.allPro === 2 || data?.allPro === 3) ? 1 : data?.allPro;
            formData.append('allPro', booleanValue);
            // if (data.pageImages.fileList){

            // }
            
            (data?.pageImages && data?.pageImages?.fileList) && data?.pageImages?.fileList?.forEach((file) => {
                if (file.url) {
    ...
    return (
        <ErrorBoundary>
            ...
            <figure className="ps-block--form-box">
                ...
                <form onSubmit={handleSubmit(submitFormHandler)} className="ps-form ps-form--new px-2">
                    ...
                    <div className="form-group w-100 mw-100">
                        <label>Write English Content:<sup>*</sup></label>
                    ...
                    <div className="form-group">
                        <label>Include Products<sup>*</sup></label>
                        <Controller
                            name="allPro"
                            control={control}
                            render={({ field }) => (
                                ...
                                <Controller
                                    ...
                                    render={({ field }) => (
                                        <Select
                                            ...
                                            options={filteredSubCategories?.[0]?.subcategory?.map((item) => ({
                                                value: item.slug,
                    ...
                    <div className="ps-form__bottom">
                        <div className="ps-btn ps-btn--black" onClick={() => (router.push('/custom-pages/web-pages'))}>
                            Back
                        </div>
