...
import { UserOutlined, FileOutlined, LineChartOutlined } from '@ant-design/icons';
...
const Card = ({ data, loading, compareData, compareEnabled }) => (
    <div className='row dash-card justify-content-between px-4 px-sm-0' style={{ columnGap: '10px' }}>
        {data.map((item, index) => {
            const compareItem = compareData?.[index];
            const showComparison = compareEnabled && compareItem && !loading;
            
            return (
                <div key={item.key} className='col-md-4 col mb-4 rounded-lg p-3 shadow-custom'>
                    <div className='d-flex align-items-center justify-content-start'>
                        <div className='color-white icon py-2 mr-3'>
                            <i className={`fa ${item.iconClass} m-0 p-0`} aria-hidden="true"></i>
                        </div>
                        <div className='flex-grow-1'>
                            <p className='m-0 p-0 text-muted' style={{ fontSize: showComparison ? '12px' : "14px" }}>
                                {item.title}
                            </p>
                            <div className='d-flex align-items-center'>
                                <h2 className='m-0 p-0' style={showComparison ? { fontSize:'14px' }:{}}>
                                    {loading ? (
                                        <Spin size="small" />
                                    ) : (
...
const CardAnalytics = ({ startDate, endDate, compareDates, compareEnabled }) => {
