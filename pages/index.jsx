...
const Index = () => {
  ...
  const compareStartDateFormatted = comparisonDates?.startDate
    ? moment(comparisonDates?.startDate).format("YYYY-MM-DD")
    : null;
  const compareEndDateFormatted = comparisonDates?.startDate
    ? moment(comparisonDates?.endDate).format("YYYY-MM-DD")
    : null;
  console.log(
    startDateFormatted,
    endDateFormatted,
    ...
    "compareStartDateFormatted, compareEndDateFormatted"
  );

  const { data: purchaseCategory, error: purchaseCategoryError } = useSWR(
    ["/purchase-category", startDateFormatted, endDateFormatted],
    () => mostPurchaseCategory(startDateFormatted, endDateFormatted)
  );
  ...
  useEffect(() => {
    if (socket) {
      ...
      const events = [
        {
          event: "branchStatusWiseCountResponse",
          key: "branchStatusWiseCount",
        },
        {
          event: "branchOpenUnopenOrdersResponse",
          key: "branchWiseOpenOrders",
        },
      ];
      socket.on("connect", () => {
  ...
  let dateView = (
    <div
      className="date-range-container d-flex align-items-center justify-content-end justify-content-sm-start "
      style={{ columnGap: "6px", rowGap: "10px" }}
    >
      <DateRangePicker
  ...
  const compareDates = {
    compareStartDate: compareStartDateFormatted,
    compareEndDate: compareEndDateFormatted,
  };
  const props = {
      ...
      ? [
          {
            label: "Recovery",
            key: "item-0",
            children: <TableRecovery {...props} />,
          },
        ]
      : []),
    {
      label: "Most Purchase Product",
      key: "item-1",
      children: <TableTopProduct {...props} />,
    },
    {
      label: "Most Purchase Category",
      key: "item-2",
      children: (
        <TableTopCategory
          error={purchaseCategoryError}
          items={purchaseCategory}
          startDate={startDateFormatted}
          endDate={endDateFormatted}
          title="Most Purchase Category"
        />
      ),
    },
    {
      label: "Call Center",
      key: "item-3",
      children: <TableCallCenter {...props} />,
    },
    {
      label: "Branch Orders",
      key: "item-4",
      children: (
        <TableBranchOrders
          items={data.branchWiseOpenOrders}
          title="Branch Orders"
        />
      ),
    },
  ...
  const BranchesOrders = (
    <CardTotalBranchesPortalOrders data={data.branchWiseOpenOrders} />
  );
  return (
    <ContainerDashboard title="Dashboard">
      ...
      <div
        className={`ps-card__header  pl-0 pl-md-2 pr-2 d-flex align-items-center justify-content-between header-content ${isSticky && (isDrawerMenuWeb ? "widthOpenMenu" : "widthCloseMenu")} ${isSticky ? "sticky-date-range" : ""}`}
      >
        <h4 className="d-md-block d-none mb-0">Sales Reports</h4>

        <h4 className="d-md-none d-block mb-0">
          {!compareEnabled ? "Date" : ""}
        </h4>
        ...
        <div
          className={`d-flex position-relative justify-content-end float-right ${!isSticky ? "mb-2 mb-md-4" : ""} `}
        >
      ...
      <section className="ps-dashboard" id="homepage">
        <div className="ps-section__left max-width">
          <div className="row">
            <div className="col-xl-12 col-12">
              ...
              <ExpoRecords
                ...
                endDate={endDateFormatted}
              />
              <div className="d-block d-md-none">{OrderCountCard}</div>
              {showfinancialDetail && (
                <SalesRecord
                  startDate={startDateFormatted}
                  endDate={endDateFormatted}
          ...
          <div className="d-lg-block d-none">{BranchesOrders}</div>
          <UpTimeRobotCard />
          <CardTopAreas {...props} />
        </div>
        <div className="ps-section__right">
          <div className="d-md-block d-none">{OrderCountCard}</div>
          <CardPortalStatus
            startDate={startDateFormatted}
            endDate={endDateFormatted}
            compareDates={compareDates}
          ...
          <CardOrderStatus
            startDate={startDateFormatted}
            endDate={endDateFormatted}
            ompareDates={compareDates}
            compareEnabled={compareEnabled}
          />
          <div className="d-lg-none d-block">{BranchesOrders}</div>
        </div>
      </section>
    </ContainerDashboard>
  );
